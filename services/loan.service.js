...
const applyForLoan = async(loanDetails, loanAmount, loanPeriod, id) => {
    try{
        ...
        throw new AppError('Error while creating Loan credentials in database', 500);
    }
};

const outstandingLoanExists = async(id) => {
    try{
        const outstandingLoan = await Loan.findOne({ userId: id, loanStatus: 'Not paid'});
...
const pullLoanDetails = async(id) => {
    try{
        ...
        throw new AppError(`Error while getting your loan details. Try again: ${error}`, 500);
    }
};
